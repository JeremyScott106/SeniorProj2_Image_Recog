...
def compute_class_metrics(all_true_labels, all_predicted_labels, num_classes):
    ...
    all_predicted_labels: List of predicted labels.
    Returns:
    superclass_metrics_list: List of metrics for each superclass.
    ...
    class_metrics_list = []
    if (num_classes == 20):
        labels = create_superclass_labels()
    if (num_classes == 100):
        labels = create_subclass_labels()
    ...
    return class_metrics_list, superclass_accuracies, macro_precision, macro_recall, macro_f1
...
def evaluate(model, model_type, num_classes, testloader, device, criterion):
    ...
    '''
    # Call the test function on model 2
    test_accuracy, all_predicted_labels, all_true_labels = test(model, testloader, device, num_classes)

    # Call compute_superclass_accuracies
    class_accuracies = compute_class_accuracies(all_predicted_labels, all_true_labels, num_classes)

    # Call compute_superclasss_metrics function
    class_metrics_list, superclass_accuracies, class_macro_precision, class_macro_recall, class_macro_f1_score = compute_class_metrics(all_true_labels, all_predicted_labels, num_classes)
    ...
    for item in sorted_class_metrics_list:
        tabledata_metrics.append([item[0], item[4], item[5], item[6]])
    
    max_len = max([len(row[0]) for row in tabledata_metrics])
    for row in tabledata_metrics:
    ...
    print(f"Macro-Precision: {class_macro_precision:.2%}")
    print(f"Macro-Recall: {class_macro_recall:.2%}")
    print(f"Macro-F1-score: {class_macro_f1_score:.2%}")
...
if __name__ == '__main__':
    ...
    for i in range(2):
        ...
        if model is not None:
            ...
            confusion_matrix = evaluate(model, model_type, num_classes, testloader, device, criterion)
            if (num_classes == 20):
